...
while($rows=mysql_fetch_assoc($result)){
	...
	print "<b>".$rows['title']."</b><br><br>";
	echo date("Y-m-d/H:i:s",$rows['time'])."<br><br>";
	print $rows['content']."<br><br>";
...
$sql="select * from comment where aid={$_GET['id']} order by time desc";
$result=mysql_query($sql);
...
if(isset($_COOKIE["qianniancc_login"])){
	print "你好，{$_COOKIE['qianniancc_login']}<br><br>";	
}
?>
